...
#class BinDecisionFork:

#    def __init__(self, attr, value, attrname=None, branches=None):
#        update(self, attr=attr, value=value, attrname=attrname or attr,
#               branches=branches or {})
...
def DecisionTreeLearner(dataset):
    ...
    def bin_decision_tree_learning(examples, attrs, parent_examples=()):
        if len(examples) == 0:
            return plurality_value(parent_examples)
        elif all_same_class(examples):
            return DecisionLeaf(examples[0][target])
            ...
            A, V = bin_choose_value(attrs, examples)
            tree = DecisionFork(A, V)
            
            for (v_k, exs) in bin_split_by(A, examples):
                subtree = bin_decision_tree_learning(
                    exs, removeall(A, attrs), examples)
                tree.add(v_k, subtree)
            return tree
    ...
    def count_not(attr, val, examples):
        return count_if(lambda e: e[attr] != val, examples)
    ...
    def bin_choose_value(attrs, examples):
        max_attr = None
        max_val = None
        max_infogain = 0
        for attr in attrs:
            for val in values[attr]:
                infogain = bin_information_gain(attr, val, examples)
        ...
        return max_attr, max_val
         ...
         #return argmax_random_tie([attrs for v in ,
              #                   lambda a: bin_information_gain(
    

    def bin_information_gain(attr, val, examples):
        def I(examples):
            return information_content([count(target, v, examples)
                                        for v in values[target]])
            #return information_content([count(target, val, examples),
            #                            count_not(target, val, examples)])

        N = float(len(examples))

        remainder = sum((len(examples_i) / N) * I(examples_i)
                        for (v, examples_i) in bin_split_by(attr, val, examples))
    ...
    def information_gain(attr, examples):
        ...
        #best_value = argmax_random_tie(between all of attr's values)
        
        remainder = sum((len(examples_i) / N) * I(examples_i)
                        for (v, examples_i) in split_by(attr, examples))
        return I(examples) - remainder

    def binary_information_gain(attr, examples):
        "Return the expected reduction in entropy from binary-splitting by attr."
        def I(examples):
            pass
        pass

    def bin_split_by(attr, val, examples):
        return [(val, [e for e in examples if e[attr] == val]),
                ('Not '+val, [e for e in examples if e[attr] != val])]
...
if __name__ == '__main__':
    ...
    ada = AdaBoost(WeightedLearner(DecisionTreeLearner), 10)(restaurant)
    test(ada, restaurant)
